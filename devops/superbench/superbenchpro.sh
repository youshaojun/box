...
about() {
	echo ""
	echo " ========================================================= "
	echo " \                 superbenchpro.sh  Script                 / "
	echo "Basic system info,Geekbench,besttrace,I/O test,Media Unlock test and speedtest"
	echo " \                   v1.3.5 (2022-02-10)                 / "
	echo " ========================================================= "
	echo ""
	echo ""
}
...
benchinit() {
	...
	if  [ ! -e './speedtest-cli/speedtest' ]; then
		...
		wget --no-check-certificate -qO speedtest.tgz https://dl.233.mba/d/yilai/speedtest.tgz > /dev/null 2>&1
		if [[ $? -ne '0' ]]; then
	...
	if [ ! -e './besttrace4/besttrace' ]; then
		...
		wget --no-check-certificate -T 10 -qO besttrace4linux.zip https://dl.233.mba/d/yilai/besttrace4linux.zip > /dev/null 2>&1
		if [[ $? -ne '0' ]]; then
			wget --no-check-certificate -O besttrace4linux.zip https://dl.233.mba/d/yilai/besttrace4linux.zip > /dev/null 2>&1
		fi
	...
	if [ ! -e './geekbench/geekbench4' ]; then
		...
		wget --no-check-certificate -qO Geekbench.tar.gz https://dl.233.mba/d/yilai/Geekbench.tar.gz > /dev/null 2>&1
	...
	if  [ ! -e 'tools.py' ]; then
		...
		wget --no-check-certificate https://dl.233.mba/d/yilai/tools.py > /dev/null 2>&1
	fi
	chmod a+rx tools.py

	if  [ ! -e 'fast_com.py' ]; then
		echo " Installing Fast.com-cli ..."
		wget --no-check-certificate https://dl.233.mba/d/yilai/fast_com.py > /dev/null 2>&1
		wget --no-check-certificate https://dl.233.mba/d/yilai/fast_com_example_usage.py > /dev/null 2>&1
	fi
...
besttrace_test() {
    if [ "$2" = "tcp" ] || [ "$2" = "TCP" ]; then
        echo -e "\nTraceroute to $4 (Mode, Max $3 Hop)" | tee -a $log
        echo -e "============================================================" | tee -a $log
...
print_intro() {
	printf ' superbenchpro.sh -- https://www.wrnxr.cn/163.html\n' | tee -a $log
	printf " Mode  : ${GREEN}%s${PLAIN}    Version : ${GREEN}%s${PLAIN}\n" $mode_name 1.3.5 | tee -a $log
	printf ' Usage : bash <(wget --no-check-certificate -O- https://dl.233.mba/sh/superbenchpro.sh)\n' | tee -a $log
}
